...
def test_chord_reaches_what_tmux_name_reaches(name):
    """
...
def test_modifier_on_key_older_table_never_named(written, key):
    """
...
def test_key_it_names_has_bytes_to_send(written):
    """
...
def test_super_hyper_and_meta_get_built_name():
...
def test_shift_on_letter_is_capital():
    assert chord("shift+a") == ("A",)
    assert chord("A") == ("A",)


def test_four_keys_that_carry_control_code():
...
def test_sequence_is_every_key_in_order():
    assert keys_of("ctrl+b w") == (Keys.ControlB, "w")


def test_prefix_is_step_and_caller_supplies_it():
    """
...
def test_prefix_without_prefix_says_so():
    with pytest.raises(ValueError, match="no prefix"):
        keys_of("prefix b")


def test_chord_is_one_press_and_sequence_is_several():
    """
...
def test_keys_keyboard_can_leave_out_come_first():
    """
...
def test_function_row_reads_in_order():
...
def test_it_completes_segment_and_leaves_chord_alone():
...
def test_it_completes_step_and_leaves_ones_before_it():
    (completion,) = offered("prefix b ho")
    assert completion.text == "home"
    assert completion.start_position == -2


def test_modifier_already_held_is_not_offered_again():
...
def test_reason_names_key_pane_reads_instead():
    said = why_pane_cannot_read(event("super+a"), Modifier.SUPER, "a")

    assert said.startswith("super+a reaches this pane as a:")
    assert "no super on that key" in said


def test_reason_for_key_with_no_form_says_so():
    event = KeyEvent(FIRST_FUNCTIONAL_KEY + 20, 0, "u")

    assert "cannot reach this pane at all" in why_pane_cannot_read(event, 0, "")
...
@pytest.mark.parametrize("name", sorted(PYMUX_TO_PROMPT_TOOLKIT_KEYS))
def test_tmux_vocabulary_reads_unchanged(name):
    """
...
def test_tmux_name_that_no_table_holds(name, key):
    """
...
def test_chord_spelling_reads_here_too():
    assert key_however_it_is_written("ctrl+home") == (Keys.ControlHome,)


def test_what_is_not_key_is_still_not_key():
    """
...
def test_it_offers_across_whole_grammar():
