...
def test_lowercase_alt_key_is_escape_and_letter():
    assert reached("M-a") == (Keys.Escape, "a")


@pytest.mark.parametrize("key", ["M-J", "M-K", "M-R"])
def test_uppercase_alt_keys_tmux_binds(key):
    """
...
def test_longer_alt_names_still_come_out_of_table(key):
    assert reached(key) == PYMUX_TO_PROMPT_TOOLKIT_KEYS[key]


def test_prefixes_are_case_blind():
...
def test_key_nobody_names_is_still_refused(key):
    """
...
def test_table_still_completes_written_out_alt_keys():
    """
...
def test_configuration_file_can_bind_uppercase_alt_key():
    "The reported line. It answered `Invalid key: 'M-J'`."
    assert bound("bind-key -n M-J swap-pane -D") == []


def test_configuration_file_still_refuses_key_nobody_names():
