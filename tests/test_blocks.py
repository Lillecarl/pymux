...
def test_how_many_pixel_rows_cell_row_needs():
...
def test_row_that_is_clear_through_draws_nothing():
    lines = blocks_for(rgba(CLEAR, CLEAR), columns=1, rows=1)
    assert lines == [""]


def test_pixel_that_is_half_clear_counts_as_clear():
...
def test_run_of_one_colour_writes_sequence_once():
...
def test_cell_that_was_stepped_over_breaks_run():
...
def test_every_row_ends_with_reset():
...
def test_image_that_is_too_small_draws_nothing():
...
def test_sixteen_colours_take_plain_codes():
...
def test_average_of_block_of_pixels():
...
def test_clear_pixel_does_not_colour_its_neighbour():
...
def test_block_of_clear_pixels_stays_clear():
    out = average_rgba(rgba(CLEAR, CLEAR), 2, 1, 1, 1)
    assert out == bytes(4)


def test_same_size_is_given_back_untouched():
    pixels = rgba(RED, GREEN)
    assert average_rgba(pixels, 1, 2, 1, 2) is pixels


def test_growing_image_repeats_pixels():
...
def test_size_of_nothing_is_no_crash(size):
