...
def test_a_pane_in_a_row_has_no_pane_of_its_own_above_it():
    ...
    window = create_window(HSplit([over, VSplit([left, right])]))
...
def test_a_stack_above_gives_the_pane_that_touches_us():
    ...
    under = create_pane()
    window = create_window(HSplit([HSplit([top, bottom]), under]))
